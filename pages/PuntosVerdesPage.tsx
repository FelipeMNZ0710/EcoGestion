...
const statusInfo: Record<LocationStatus, { text: string; color: string }> = {
    ok: { text: "Operativo", color: "text-emerald-600 bg-emerald-100" },
    reported: { text: "Reportado", color: "text-amber-600 bg-amber-100" },
    maintenance: { text: "En Mantenimiento", color: "text-blue-600 bg-blue-100" },
    serviced: { text: "Servicio Reciente", color: "text-cyan-600 bg-cyan-100" }
};
...
const LocationCard = forwardRef<HTMLDivElement, {
    ...
    return (
        <div ref={ref} className={`location-card ${isSelected ? 'is-selected' : ''} ${isHovered ? 'is-hovered' : ''}`} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} onClick={onClick} role="button" tabIndex={0} aria-label={`Ver detalles de ${location.name}`}>
            ...
            {user && (
                <button onClick={(e) => { e.stopPropagation(); onToggleFavorite(); }} className={`absolute top-2 left-2 z-10 p-1.5 rounded-full transition-colors ${isFavorite ? 'text-yellow-400 bg-yellow-400/20' : 'text-slate-400 bg-white/50 hover:text-yellow-400'}`} title={isFavorite ? 'Quitar de favoritos' : 'Añadir a favoritos'}>
            ...
            <div className="flex flex-col flex-grow py-1">
                ...
                <div className="flex items-center gap-1.5 mt-2"><div className="flex flex-wrap gap-1">{location.materials.map(material => (<span key={material} className="px-1.5 py-0.5 text-xs bg-slate-100 text-slate-700 font-medium rounded-md">{material}</span>))}</div></div>
            </div>
        </div>
    );
});
        ...
        <div className="modal-backdrop" onClick={onClose}>
            <div className="modal-content" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-6">
                    ...
                    <h2 className="text-2xl font-bold text-text-main mt-3">{location.name}</h2>
                    ...
                    <div className="mt-4 border-t pt-4">
                        ...
                        <div className="flex flex-wrap gap-2">{location.materials.map(m => (<span key={m} className="px-2 py-1 text-sm bg-slate-100 text-slate-800 font-medium rounded-md">{m}</span>))}</div>
                    </div>
                    {user && (
                        <div className="mt-6 flex gap-3">
                            ...
                            <button onClick={onReport} className="flex-1 bg-amber-100 text-amber-800 font-bold py-3 rounded-lg hover:bg-amber-200 transition-colors">Reportar Problema</button>
                        </div>
                    )}
                </div>
            </div>
            ...
            <div className="modal-content" onClick={e => e.stopPropagation()}>
                <div className="p-6">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4 modal-form">
                        ...
                        <div className="flex justify-end space-x-3 pt-4"><button type="button" onClick={onClose} className="px-4 py-2 bg-slate-200 text-slate-800 rounded-md hover:bg-slate-300">Cancelar</button><button type="submit" className="px-4 py-2 bg-primary text-white rounded-md hover:bg-primary-dark">Guardar Cambios</button></div>
                    </form>
                </div>
            </div>
        </div>
...
const PuntosVerdesPage: React.FC<{ user: User | null; setUser: (user: User | null) => void; onUserAction: (action: GamificationAction, payload?: any) => void; isAdminMode: boolean; }> = ({ user, setUser, onUserAction, isAdminMode }) => {
    ...
    return (
        <div className="bg-background lg:h-[calc(100vh-80px)] lg:flex lg:flex-col">
            ...
            <header className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full text-center py-12"><h1 className="text-4xl font-extrabold text-text-main sm:text-5xl">Encontrá tu Punto Verde</h1><p className="mt-4 text-lg text-text-secondary max-w-3xl mx-auto">Utilizá los filtros y el mapa interactivo para localizar el centro de reciclaje más conveniente.</p></header>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full lg:flex-1 lg:overflow-hidden pb-12">
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 lg:h-full">
                    ...
                    <main className="lg:col-span-3 lg:h-full flex flex-col">
                        <div className="flex-shrink-0 p-4 bg-white/80 rounded-xl border border-slate-200/80 shadow-sm backdrop-blur-sm space-y-4">
                            <div className="relative"><svg className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M9 3.5a5.5 5.5 0 100 11 5.5 5.5 0 000-11zM2 9a7 7 0 1112.452 4.391l3.328 3.329a.75.75 0 11-1.06 1.06l-3.329-3.328A7 7 0 012 9z" clipRule="evenodd" /></svg><input type="search" placeholder="Buscar por nombre o dirección..." className="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-secondary focus:outline-none transition" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /></div>
                        ...
                        <div className="flex-1 overflow-y-auto space-y-4 pt-4 pr-2 mt-4 relative">
                            ...
                            {filteredAndSortedLocations.length === 0 && (<div className="text-center py-16"><svg className="mx-auto h-12 w-12 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path vectorEffect="non-scaling-stroke" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 13h6m-3-3v6m-9 1V7a2 2 0 012-2h6l2 2h6a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2z" /></svg><h3 className="mt-2 text-lg font-semibold text-slate-900">Sin resultados</h3><p className="mt-1 text-sm text-slate-500">No se encontraron puntos verdes con esos filtros.</p></div>)}
                        </div>
                    </main>
                </div>
            </div>
