...
const secondUserBatchRaw = [
    ...
    "76 ALMIRÓN AIELET ÁMBAR 47102940", "77 AMARILLA JULIO CÉSAR"
];
...
const messagesData: Record<string, Omit<Message, 'avatarInitials' | 'avatarColor' | 'id'>[]> = {
    general: [
        ...
        { user: 'Admin Recicla', time: '01:15 PM', text: 'Hola, por ahora sigue en pie. Si hay cambios avisaremos en el canal de #anuncios y en redes. ¡Gracias por el interés!'},
    ],
    anuncios: [
    ...
    dudas: [
        ...
        { user: 'Admin Recicla', time: '02:42 PM', text: '¡Excelente pregunta, Laura! Esas lamparitas contienen mercurio y no van a la basura común. Hay que llevarlas a puntos de recolección de residuos peligrosos. Pronto actualizaremos el mapa con esos puntos.' },
        ...
        { user: 'Admin Recicla', time: '03:18 PM', text: 'Hola Anahi, lamentablemente el telgopor es muy difícil de reciclar por ahora y no lo recibimos en los Puntos Verdes. Lo mejor es evitar su consumo si es posible.' },
        { user: 'Vallejos Ignacio Alejandro', time: '04:05 PM', text: '¿Y las radiografías viejas? ¿Qué hago con eso? Tengo un montón.' },
        { user: 'Admin Recicla', time: '04:07 PM', text: 'Buena pregunta, Ignacio. Las radiografías contienen plata y plásticos que no se reciclan de forma convencional. Hay programas específicos que las recuperan. Estamos gestionando un punto de acopio, ¡avisaremos por acá!' },
    ],
    proyectos: [
        ...
        { user: 'Alvarez Luana', time: '09:35 AM', text: '¡Qué buena iniciativa, Marcos! Yo estoy haciendo ecoladrillos con los plásticos de un solo uso. Si alguien junta, me avisa y los paso a buscar.' },
        { user: 'Benítez Gonzalo', time: '10:02 AM', text: 'Luana, yo junto un montón de paquetes de fideos y galletitas. Te los guardo. ¿Sirven?' },
        { user: 'Alvarez Luana', time: '10:05 AM', text: '¡Sí, esos son perfectos! Gracias Gonzalo.'},
    ...
    compostaje: [
        ...
        { user: 'Garrido Amín', time: 'Ayer a las 5:30 PM', text: 'Hola Juana! A mí me pasaba lo mismo. Probablemente le falten \'secos\'. Agregale hojas secas, cartón en pedacitos o aserrín. Eso equilibra la humedad y le da aire.' },
...
const ComunidadPage: React.FC<ComunidadPageProps> = ({ user }) => {
    ...
    const sortedMembers = useMemo(() => {
        const currentMembers = members.filter(m => m.name !== 'Monzón Felipe'); // Filter out logged in user from list
    ...
    return (
        <div className="flex h-[calc(100vh-80px)] font-sans bg-gray-100 text-gray-800">
            ...
            <aside className="w-64 bg-gray-800 text-gray-300 flex flex-col">
                <header className="p-4 border-b border-gray-700 font-bold text-white text-lg">
                    Canales
                </header>
                <nav className="flex-1 overflow-y-auto p-2 space-y-1">
                        ...
                        <div key={channel.id} className="group relative">
                            ...
                            {isAdmin && channel.id !== 'general' && (
                                <div className="absolute right-1 top-1/2 -translate-y-1/2 hidden group-hover:flex items-center bg-gray-700 rounded">
                                    <button onClick={() => handleMoveChannel(channel.id, 'up')} disabled={index <= 1} className="p-1 hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed">
                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 15l7-7 7 7" /></svg>
                                    </button>
                                    <button onClick={() => handleMoveChannel(channel.id, 'down')} disabled={index >= channels.length - 1} className="p-1 hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed">
                 ...
                 {isAdmin && (
                    <div className="p-2">
                        <button onClick={() => setCreateModalOpen(true)} className="w-full text-center px-3 py-2 rounded bg-gray-700 hover:bg-gray-600 transition-colors">
                            Añadir Canal +
                        </button>
                    </div>
                )}
                    ...
                    {(messages[activeChannel] || []).map(message => (
                         <div key={message.id} className={`group flex items-start space-x-3 ${user && message.user === user.name ? 'justify-end' : ''}`}>
                             {user && message.user !== user.name && (
                                <div className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm ${message.avatarColor}`}>
                                ...
                                {editingMessageId === message.id ? (
                                    <div className="w-full">
                                        <input
                                            ...
                                            className="w-full px-2 py-1 border border-secondary rounded-md"
                                            onKeyDown={(e) => {
                                    ...
                                    <div className="flex flex-wrap gap-1 mt-1">
                                        {Object.entries(message.reactions).map(([emoji, users]) => (
                                            <button 
                                                ...
                                                className={`px-2 py-0.5 rounded-full text-sm flex items-center space-x-1 transition-colors ${user && users.includes(user.name) ? 'bg-secondary/30 border border-secondary' : 'bg-gray-200 hover:bg-gray-300 border border-transparent'}`}
                                            >
                                                <span>{emoji}</span>
                            ...
                            {user && (
                                <div className="relative">
                                    <div className="hidden group-hover:flex items-center bg-white border rounded-md shadow-sm -mt-2">
                                        <button onClick={() => { setActiveReactionPicker(message.id); setActiveMessageMenu(null); }} className="p-1.5 text-gray-500 hover:text-gray-800 hover:bg-gray-100 rounded-l-md">
                                            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M10 18a8 8 0 100-16 8 8 0 000 16zM7 9a1 1 0 100-2 1 1 0 000 2zm7-1a1 1 0 11-2 0 1 1 0 012 0zm-.464 5.535a.5.5 0 00.707 0l2-2a.5.5 0 00-.707-.707l-1.646 1.647-.07-.07a3.5 3.5 0 00-4.95 0l-.07.07-1.647-1.647a.5.5 0 00-.707.707l2 2a.5.5 0 00.707 0z" /></svg>
                                        </button>
                                        <button onClick={() => { setActiveMessageMenu(message.id); setActiveReactionPicker(null); }} className="p-1.5 text-gray-500 hover:text-gray-800 hover:bg-gray-100 rounded-r-md">
                                            ...
                                            {activeReactionPicker === message.id && (
                                                <div className="p-1 flex justify-around">
                                                    {availableReactions.map(emoji => (
                                                        <button key={emoji} onClick={() => handleToggleReaction(message.id, emoji)} className="text-xl p-1 rounded-md hover:bg-gray-200">{emoji}</button>
                                                    ))}
                                                </div>
                                            )}
                                            {activeMessageMenu === message.id && (
                                                <div className="p-1">
                                                    {message.user === user.name && <button onClick={() => handleStartEditing(message)} className="w-full text-left px-2 py-1.5 rounded hover:bg-gray-100">Editar Mensaje</button>}
                                                    {isAdmin && <button onClick={() => handleDeleteMessage(message.id)} className="w-full text-left px-2 py-1.5 rounded text-red-600 hover:bg-red-50">Eliminar Mensaje</button>}
                                                </div>
                                            )}
                                        </div>
                                    )}
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-2">
                    {sortedMembers.map(member => (
                        <div key={member.name} className="flex items-center space-x-3 p-1">
                            ...
                            {ADMIN_USERS.includes(member.name) && (
                                <span className="px-1.5 py-0.5 text-xs font-semibold rounded-full bg-accent/30 text-amber-800">Admin</span>
                            )}
                        </div>
                    ))}
                </div>
